/* 
QUESTÃO: 399. Evaluate Division
LINK: https://leetcode.com/problems/evaluate-division/description/?envType=problem-list-v2&envId=graph
...
var calcEquation = function(equations, values, queries) {
    ...
    const dfs = (src, dest, visited) => {
        if (!graph.has(src) || !graph.has(dest)) return -1;
        if (src === dest) return 1;

        visited.add(src);

        for (const [neighbor, value] of graph.get(src)) {
